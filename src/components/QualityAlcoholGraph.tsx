...
interface QualityAlcoholGraphProps {
  wineData: WineData;
  score: number;
  width: number;
  height: number;
...
// Function to predict wine quality (simplified version of the one in PredictPage)
const predictQualityByAlcohol = (baseWineData: WineData, alcoholContent: number): number => {
  // Create a copy of the wine data with the new alcohol content
  const wineData = { ...baseWineData, alcohol: alcoholContent };
...
const QualityAlcoholGraph: React.FC<QualityAlcoholGraphProps> = ({ 
  wineData, 
  score, 
  width, 
  height 
}) => {
  // Generate data points for different alcohol percentages
  const data = useMemo(() => {
    const alcoholRange = Array.from({ length: 61 }, (_, i) => 8 + (i * 0.1)); // 8% to 14% in 0.1% increments
    return alcoholRange.map(alcohol => ({
      alcohol,
      quality: predictQualityByAlcohol(wineData, alcohol)
    }));
  }, [wineData]);
  ...
  const xScale = scaleLinear({
    domain: [8, 14], // Alcohol percentage range
    range: [0, innerWidth],
    nice: true
  });
  ...
  // Accessors
  const getX = (d: { alcohol: number; quality: number }) => xScale(d.alcohol);
  const getY = (d: { alcohol: number; quality: number }) => yScale(d.quality);
...
export default QualityAlcoholGraph;